...
import { indexOfStartingFrom as indexFrom } from "./functions/index_starting_from";

export const convertHookWidgetCommandName = "extension.convert-hook-widget";

export function convertHookWidgetCommandHandler(document: vscode.TextDocument, range: vscode.Range) {
  ...
  const widgetClassDefinitionLineText = documentTextArray[widgetClassDefinitionLineNumber];
  const widgetClassDefinitionLineRange = new vscode.Range(
    new vscode.Position(widgetClassDefinitionLineNumber, 0),
    new vscode.Position(widgetClassDefinitionLineNumber, widgetClassDefinitionLineText.length)
  );

  const widgetClassDefinitionLineMatch = widgetClassDefinitionLineText.match(classDefinitionRegex) as RegExpMatchArray;
  ...
  const buildMethodLineNumber = indexFrom(documentTextArray, buildMethodRegex, widgetClassDefinitionLineNumber);
  ...
  const refBuildMethodLineText = buildMethodLineText.replace(buildMethodRegex, "Widget build(BuildContext context)");

  const edit = new vscode.WorkspaceEdit();

  insertImportStatement(edit, document, documentTextArray, "import 'package:flutter_hooks/flutter_hooks.dart';");
